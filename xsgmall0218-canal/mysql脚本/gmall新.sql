/*
SQLyog 
MySQL - 5.7.25-log : Database - gmall0105
*********************************************************************
*/
...
insert  into `base_province`(`id`,`name`,`region_id`) values 
(1,'北京','1'),
(2,'上海','2'),
(3,'广东','5'),
(4,'黑龙江','3'),
...
CREATE TABLE `order_detail` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='订单明细表';

/*Data for the table `order_detail` */

insert  into `order_detail`(`id`,`order_id`,`sku_id`,`sku_name`,`img_url`,`order_price`,`sku_num`) values 
(1,1,9,'荣耀10 GT游戏加速 AIS手持夜景 6GB+64GB 幻影蓝全网通 移动联通电信','http://QITsgEEhYuekPrqBMenZlePJetDMZLJajQEzITvt',2452.00,'4'),
(2,1,4,'小米Play 流光渐变AI双摄 4GB+64GB 梦幻蓝 全网通4G 双卡双待 小水滴全面屏拍照游戏智能手机','http://zBhgjDpMRYsRgecbYMQUbylKUqXNYCRDqUzOuHco',1442.00,'2'),
(3,1,7,'荣耀10青春版 幻彩渐变 2400万AI自拍 全网通版4GB+64GB 渐变蓝 移动联通电信4G全面屏手机 双卡双待','http://kimIEXYcpqMOJPgjFxxSZrNOFJJeCASMhBkvzjwH',1553.00,'5'),
(4,1,7,'荣耀10青春版 幻彩渐变 2400万AI自拍 全网通版4GB+64GB 渐变蓝 移动联通电信4G全面屏手机 双卡双待','http://ihjyQKciJIxmSCjoPjBcKtONzgiySUSFyCoPjxNx',1553.00,'5');
...
CREATE TABLE `order_info` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='订单表 订单表';
...
(1,'MtHeBs','13174808736',106.00,'1',1,'2','gLjPxRQCqYRriRIRlBaz','ssLBAWfODATPuDHCQvJT','4853291939','','2019-06-28 06:40:45',NULL,NULL,NULL,NULL,NULL,6);
...
(10,1,222,'小米（MI） 小米路由器4 双千兆路由器 无线家用穿墙1200M高速双频wifi 千兆版 千兆端口光纤适用','小米（MI） 小米路由器4 双千兆路由器 无线家用穿墙1200M高速双频wifi 千兆版 千兆端口光纤适用',1.24,101,329,'http://kAXllAQEzJWHwiExxVmyJIABfXyzbKwedeofMqwh','2019-02-14 21:25:54');
...
/*!50003 CREATE DEFINER=`root`@`localhost` FUNCTION `rand_num`(from_num INT ,to_num INT) RETURNS int(11)
BEGIN   
...
/*!50003 CREATE DEFINER=`root`@`localhost` FUNCTION `rand_nums`(from_num INT ,to_num INT,n INT ,delemit VARCHAR(20)) RETURNS varchar(255) CHARSET utf8
BEGIN   
 DECLARE i INT DEFAULT 0;  
...
/*!50003 CREATE DEFINER=`root`@`localhost` FUNCTION `rand_num_seed`(from_num INT ,to_num INT,seed LONG ) RETURNS int(11)
BEGIN   
 DECLARE i INT DEFAULT 0;  
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `init_data`( do_date_string VARCHAR(20) ,order_incr_num INT,user_incr_num INT   ,if_truncate BOOLEAN  )
BEGIN  
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_order`( create_time_string VARCHAR(200),max_num INT,user_num INT   )
BEGIN  
 ...
 VALUES (rand_string(6) , CONCAT('13',rand_nums(0,9,9,'')),CAST(rand_num(50,1000) AS DECIMAL(10,2)) ,v_order_status ,rand_num(1,user_num), rand_num(1,2),rand_string(20),rand_string(20),rand_nums(0,9,10,''),'',v_create_time, v_operate_time,NULL,NULL,NULL,NULL ,v_province); 
  SELECT  LAST_INSERT_ID() INTO v_order_id ;
    ...
    WHILE j<v_order_detail_num DO
       ...
       select v_order_id ,id,sku_name,CONCAT('http://',rand_string(40)),price,rand_num(1,5)   from sku_info where id=v_skuid;
    END WHILE;
    SET j=0;
 UNTIL i = max_num  
 END REPEAT;  
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_payment`( do_date_str VARCHAR(200)   )
BEGIN      
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_sku`( create_time_string VARCHAR(200),max_num INT  )
BEGIN  
 DECLARE v_create_time DATETIME DEFAULT NULL;
 DECLARE i INT DEFAULT 0;
 SET autocommit = 0;    
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_user`( create_time_string VARCHAR(200),max_num INT,if_truncate boolean  )
BEGIN  
 DECLARE v_create_time DATETIME DEFAULT NULL;
 DECLARE i INT DEFAULT 0;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `update_order`(operate_time_string VARCHAR(20))
BEGIN 
  DECLARE v_operate_time DATETIME DEFAULT NULL; 
